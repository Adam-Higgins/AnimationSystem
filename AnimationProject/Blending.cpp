...
void Add(Pose& output, Pose& inPose, Pose& addPose, Pose& additiveBasePose, int blendroot)
{
	...
	for (int i = 0; i < numJoints; ++i)
	{
		...
		Transform result(input.position + (additive.position - additiveBase.position), normalized(input.rotation * (inverse(additiveBase.rotation) * additive.rotation)), input.scale + (additive.scale - additiveBase.scale));
