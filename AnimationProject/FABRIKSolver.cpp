...
{
	Transform joint = GetGlobalTransform(i);
	Transform parent = GetGlobalTransform(i - 1);

	vec3 currentHinge = joint.rotation * axis;
	vec3 desiredHinge = parent.rotation * axis;

	//mIKChain[i].rotation = mIKChain[i].rotation * fromToRotation(currentHinge, desiredHinge);
}
